...
<div class="card" style="max-width: 640px;">
    ...
    <div class="alert" style="background: rgba(79,142,247,.08); border: 1px solid rgba(79,142,247,.2); color: var(--accent); margin-bottom: 1.5rem;">
        <span>ℹ</span>
        <span>Solo se permiten correos de <strong>Gmail (@gmail.com)</strong> para todos los usuarios del sistema.</span>
    </div>
    ...
    <form method="POST" action="{{ route('admin.users.store') }}" id="createUserForm" novalidate>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                @error('username')
                    <div class="form-error">✕ {{ $message }}</div>
                @enderror
                <div class="text-xs text-muted" style="margin-top: .3rem;">Solo letras, números y _ (sin espacios)</div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div style="position: relative;">
                <input
                    ...
                    id="emailInput"
                >
                <span id="emailIndicator" style="position:absolute; right:12px; top:50%; transform:translateY(-50%); font-size:1.1rem; display:none;"></span>
            </div>
            @error('email')
                <div class="form-error">✕ {{ $message }}</div>
            @enderror
            ...
            <div class="form-group">
                ...
                <div style="position: relative;">
                    <input
                        ...
                        minlength="8"
                    >
                    <button type="button" onclick="togglePassword('password')" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--muted);font-size:.9rem;">
                        👁
                    </button>
                </div>
                @error('password')
            ...
            <div class="form-group">
                ...
                <div style="position: relative;">
                    <input
                        ...
                        minlength="8"
                    >
                    <button type="button" onclick="togglePassword('password_confirmation')" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--muted);font-size:.9rem;">
                        👁
                    </button>
                </div>
        ...
        <div style="margin-top:-.75rem; margin-bottom:1rem;">
            <div style="display:flex; gap:4px; margin-bottom:.25rem;" id="strengthBars">
                <div style="height:4px; flex:1; border-radius:2px; background:var(--border);" id="bar1"></div>
                <div style="height:4px; flex:1; border-radius:2px; background:var(--border);" id="bar2"></div>
                <div style="height:4px; flex:1; border-radius:2px; background:var(--border);" id="bar3"></div>
                <div style="height:4px; flex:1; border-radius:2px; background:var(--border);" id="bar4"></div>
            </div>
            <div class="text-xs text-muted" id="strengthLabel"></div>
        </div>
...
passwordInput.addEventListener('input', function () {
    ...
    const colors = ['var(--danger)', '#f59e0b', '#3b82f6', 'var(--success)'];
    ...
    strengthLabel.style.color = score > 0 ? colors[score - 1] : 'var(--muted)';
});
